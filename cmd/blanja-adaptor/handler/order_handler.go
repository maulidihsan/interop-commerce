...
func (s *server) GetOrders(ctx context.Context, in *v1.UserId) (*v1.Orders, error) {
	...
	response1, err := c1.GetOrders(ctx, in)
	if err != nil {
		log.Fatalf("Request failed: %v", err)
	}
	response2, err := c2.GetOrders(ctx, in)
	if err != nil {
		log.Fatalf("Request failed: %v", err)
	}

	response := append(response1.GetOrders(), response2.GetOrders()...)
	res := v1.Orders{
		Orders: response,
	}
...
func (s *server) CreateOrder(ctx context.Context, in *v1.Order) (*v1.Response, error) {
	...
	fmt.Println("Creating Order for ", in.Vendor.String())
